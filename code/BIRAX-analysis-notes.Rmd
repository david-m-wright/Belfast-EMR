...
bind_rows(
    oct_history %>% 
      ...
      mutate(`Received` = if_else(`OCT exams` < 5, "<5 per year", ">=5 per year"),
             Eyes = " ") %>% 
...
bind_rows(
  ...
  inner_join(snapshots %>% 
                   filter(follow_up_year != "0"), by = c("treatment_year" = "follow_up_year")) %>% 
  mutate(`Received` = if_else(Injections < 5, "<5 per year", ">=5 per year"),
    Eyes = " ") %>% 
...
bind_rows(
    treatment_intervals %>% 
      ...
      summarise(`Treatment intervals` = n(), 
                interval_gt80 = any(treatment_interval_days > 80), .groups = "drop") %>% 
      inner_join(snapshots %>% 
                   filter(follow_up_year != "0"), by = c("treatment_year" = "follow_up_year")) %>% 
      mutate(`Extended interval (>80 days)` = if_else(interval_gt80, "Yes", "No"),
             Eyes = " ") %>% 
      select(treatment_year, `Treatment intervals`,  `Extended interval (>80 days)`, Eyes) %>% 
      GenerateDescriptives(col_var = treatment_year),
