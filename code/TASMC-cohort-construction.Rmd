...
Three data files were provided, `MDC_Updated_16062023_anon_david.xslx` contained the clinical records of all injections, OCT scans and VA measurements (MD Clone data). `Notal+MDC with additional IDs 16062023_anon_david.xslx` and `Results Notal_with original IDs_and_anon_185.xslx` contained the fluid measurements derived from OCT scans using the Notal Ophthalmic Analyser (NOA). All files were converted to .csv format before further processing.
...
EMR for a total of `r nrow(distinct(mdc_raw, ID_anon))` patients were included in the extracted dataset, with `r nrow(injections_raw)` records of anti-VEGF injections. The unit of analysis is the eye but some criteria were applied at the patient level:   

* Excluded `r nrow(filter(distinct(mdc_raw, ID_anon, exclude_research_patient, exclude_covid), exclude_research_patient))` patients that were involved in a clinical trial and so received different treatments from the majority of patients.     
* Excluded `r nrow(filter(distinct(mdc_raw, ID_anon, exclude_research_patient, exclude_covid), !exclude_research_patient, exclude_covid))` patients that were first treated during the Covid pandemic, a substantially different group of patients.    
...
* `r nrow(filter(eye_raw, !exclude_age, exclude_lt3_injections))` eyes had fewer than three anti-VEGF injections during the study period  and were excluded.   
* `r nrow(filter(eye_raw, !exclude_age,  !exclude_lt3_injections, exclude_no_fluid))` eyes did not have a retinal fluid measurement within 40 days prior to or including the index date and were excluded.   

A total of `r nrow(eye)` eyes from `r nrow(distinct(eye, ID_anon))` patients remained after these exclusions. A total of `r nrow(injections)` injections were observed for the cohort, allowing `r nrow(treatment_intervals)` treatment intervals to be observed (between injections). 
...
```{r follow-up-by-year}
injections %>% 
  count(ID_anon, EYE, treatment_year, name = "Injections") %>% 
...
injections %>% 
  ...
  fluid_history %>% 
  filter(baseline) %>% 
  select(ID_anon, EYE, amd_type)
  , by = c("ID_anon", "EYE")) %>% 
  group_by(amd_type) %>% 
  count(amd_type, ID_anon, EYE, treatment_year, name = "Injections") %>% 
...
va_history %>% 
  ...
  count(ID_anon, EYE, treatment_year, name = "VA measurements") %>% 
...
va_history %>% inner_join(
  ...
  group_by(amd_type) %>% 
  filter(treatment_year != "0") %>% 
  count(amd_type, ID_anon, EYE, treatment_year, name = "VA measurements") %>% 
...
va_history %>% 
  filter(baseline),
by = c("ID_anon", "EYE")) %>% 
  transmute(amd_type, va_logmar, Eyes = "n") %>% 
...
fluid_history %>% 
  filter(baseline) %>% 
  inner_join(eye, by = c("ID_anon", "EYE")) %>% 
