...
* Identification of eyes with a diagnosis of AMD (`CoPathologyCode` "R316" or "R337" in co-pathology table, or `DiagnosisDescription` containing "AMD" or "age-related macular degeneration" in diagnosis table). Those with "suspected AMD" or "dry AMD" at first diagnosis were excluded (those developing dry AMD later were not excluded)).   
...
* `r nrow(filter(eye_raw, !exclude_no_AMD, !exclude_age, exclude_age_perturb))` eyes where perturbation (to protect patient privacy) meant that follow-up periods could not be calculated (gave negative follow-up periods).   
* `r nrow(filter(eye_raw, !exclude_no_AMD, !exclude_age,  !exclude_age_perturb, exclude_lt3_injections))` eyes had fewer than three anti-VEGF injections during the study period  and were excluded.   
* `r nrow(filter(eye_raw, !exclude_no_AMD, !exclude_age,  !exclude_age_perturb, !exclude_lt3_injections, exclude_RVO))` eyes had a diagnosis of RVO at any point during the study period and were excluded.   
* `r nrow(filter(eye_raw, !exclude_no_AMD, !exclude_age,  !exclude_age_perturb, !exclude_lt3_injections, !exclude_RVO, exclude_DR_DMO))` eyes had a diagnosis of DMO/DR at any point during the study period and were excluded.   
* `r nrow(filter(eye_raw, !exclude_no_AMD, !exclude_age,  !exclude_age_perturb, !exclude_lt3_injections, !exclude_RVO, !exclude_DR_DMO, exclude_no_va))` eyes did not have a visual acuity assessment within 2 weeks prior to or including the index date and were excluded.   
...
eye %>%
  ...
  kable_paper()
```
A key characteristic of the dataset is the different lengths of follow-up overall (i.e. time period for which records were extracted), in terms of treatment duration and in terms of monitoring for each examination type, given in Table \@ref(tab:follow-up-length). 
...
fluid_history %>% 
  ...
  select(Age, all_of(noa_general)) %>% 
    mutate(Eyes = " ") %>% 
   GenerateDescriptives(col_var = Age, type = c(FramesNum = "Median (Range)", NumBscanwtihFluid = "Median (Range)")) %>% 
  left_join(noa_dictionary %>% 
                 select(Parameter, `Raw Parameter`), 
              by = c("Variable" = "Raw Parameter")) %>% 
   relocate(Parameter, .after = Variable) %>% 
...
```{r fluid-baseline-by-age}
fluid_history %>% 
  filter(baseline) %>% 
  inner_join(eye, by = c("PatientID", "EyeCode")) %>% 
  select(Age, all_of(noa_fluid)) %>% 
    mutate(Eyes = " ") %>% 
   ...
   left_join(noa_dictionary %>% 
              ...
              by = c("Variable" = "Raw Parameter")) %>% 
   relocate(Parameter, .after = Variable) %>% 
   kbl(caption = "Distribution of NOA OCT fluid measurements at baseline by age category.") %>% 
   kable_paper()
```
```{r fluid-baseline-by-gender}
fluid_history %>% 
  filter(baseline) %>% 
  inner_join(eye, by = c("PatientID", "EyeCode")) %>% 
  select(Gender, all_of(noa_fluid)) %>% 
    mutate(Eyes = " ") %>% 
   ...
   left_join(noa_dictionary %>% 
              ...
              by = c("Variable" = "Raw Parameter")) %>% 
   relocate(Parameter, .after = Variable) %>% 
   kbl(caption = "Distribution of NOA OCT fluid measurements at baseline by gender.") %>% 
   kable_paper()
```
...
```{r fluid-grid-plot, fig.cap = "Distribution of retinal fluid at baseline by ETDRS grid region: mean (standard deviation).", fig.height=12, fig.width=8}
baseline_fluid_grid %>% 
  filter(str_detect(abbr_metric, "SRF|IRF|TRF")) %>% 
  PlotETDRSGrid(valform) +
  facet_wrap(~abbr_metric, ncol = 2)
```
```{r RPE-grid-plot, fig.cap = "Distribution of RPE irregularities at baseline by ETDRS grid region: mean (standard deviation).", fig.height=4, fig.width=8}
baseline_fluid_grid %>% 
...
```{r retina-grid-plot, fig.cap = "Distribution of retinal thickness at baseline by ETDRS grid region: mean (standard deviation).", fig.height=4, fig.width=8}
baseline_fluid_grid %>% 
...
cor.test(~IRFVolumeNl + SRFVolumeNl, data = filter(fluid_history, baseline))
```

## Types of AMD by fluid presence or absence
...
fluid_history %>% filter(baseline) %>% 
 count(SRF, IRF) %>% 
  ...
  adorn_ns("front") %>% 
  kbl(caption = "Presence/absence distribution of IRF and SRF.") %>% 
  kable_paper()
```

# Descriptive statistics during follow-up

Proportion injected by month of follow-up and AMD type.
Cumulative injections by month?

Probability of fluid by month of follow-up.

Quantity of fluid by month of follow-up (among those with fluid)


Clustering by fluid trajectory - supervised or unsupervised?

```{r fluid-history}

fluid_history %>% 
  ggplot(aes(x = months_since_index, y = IRFVolumeNl)) +
  geom_point() + 
  geom_smooth() + 
  lims(y = c(0, 3000))

fluid_history %>%
  filter(complete_months <= 60, IRF == 1) %>% 
  mutate(across(complete_months, as.factor)) %>% 
  ggplot(aes(x = complete_months, y = IRFVolumeNl)) +
  geom_boxplot() +
  lims(y = c(0, 1000))
